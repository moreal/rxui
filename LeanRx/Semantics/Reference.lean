...
/-- Initialize every derived and sink cache by full recomputation. -/
def initState (program : Program) (sourceStore : Store) : State :=
  let store := runDerived program.derived sourceStore
  { store, sinkCache := observe program.sinks store }
...
def run (program : Program) (old : State) (transaction : SourceTransaction) : RunResult :=
  ...
  let trace := (transaction.changedIds program old.store).map TraceEvent.sourceChanged ++
    program.derived.map (TraceEvent.derivedEvaluated ∘ (·.id)) ++
    program.sinks.map (TraceEvent.sinkEvaluated ∘ (·.name))
  { store := finalStore
