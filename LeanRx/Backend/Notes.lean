...
private def renderFunction (runtime : RuntimeNames) : Except Js.Error Function := do
  ...
  let error ← Ident.checked "effectError"
  ...
  pure {
    ...
    body := #[
      ...
      .const error (stateAt state 8),
      .const status (.conditional
        (.unary .not <| equals (.ident error) (.literal (.string "")))
        (.ident error) fallback),
    ...
    body := #[
      ...
      .ifThen condition <| .ofList <| [
        ...
        .ifThen (.unary .not <| property (.ident result) "ok") <| .ofList <|
          ...
          assignState state metrics 8 (.binary .add (.literal (.string "Save failed: "))
            (property (property (.ident result) "error") "message")) ++ [
...
private def inputFunction (render debounce : Ident) : Except Js.Error Function := do
  ...
  let body : List Stmt := [
    ...
    .ifThen (equals (stateAt state 6) (uint 1)) <| .ofList <|
      [.expr <| method (.ident effects) "cancel" [stateAt state 7]] ++
      assignState state metrics 6 (uint 4) ++ [trace metrics "command:storageGet:cancel"],
    .ifThen (.binary .or (equals (stateAt state 2) (uint 1))
        (equals (stateAt state 2) (uint 2))) <| .ofList <| [
      .expr <| method (.ident effects) "cancel" [stateAt state 3],
...
private def mountFunction (runtime : RuntimeNames) (componentName : String)
  ...
  pure {
    ...
    body := #[
      ...
      .const statusText (call runtime.createText [.literal (.string "Loading")]),
