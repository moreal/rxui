...
def size : Schema.{u} → Nat
  | .empty => 0
  | .field _ _ tail => tail.size + 1

/-- Field names in declaration order. -/
def names : Schema.{u} → List String
  | .empty => []
