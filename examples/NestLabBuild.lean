...
def generateInto (directory : System.FilePath) : IO Unit := do
  match Blip_spec.check, Chip_spec.check, Cuff_spec.check, Tick_spec.check,
      Pulse_spec.check, NestLab_spec.check with
  ...
  | _, _, .error error, _, _, _ =>
      throw <| IO.userError s!"Cuff component invalid: {error.code}"
