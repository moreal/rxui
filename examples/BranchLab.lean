...
one `detach` plus one `append` of the freshly built branch — no new host
export and no runtime ABI change. The edit input reflects the `draft` row
field into its `value` property (the sealed row reflection, reusing
ADR-0038's WHATWG equal-value caret no-op), and typing flows through the
ADR-0046 delegated `input` payload: Edit copies `label` into `draft` and
enters the edit branch, `retype (value : String)` drains one retained-row
`updateAt` per keystroke with the caret preserved, and OK commits `draft`
back into `label` and returns to the view branch — the TodoMVC edit/view
transition with retained row identity. Delegated bindings stay static across
branches: the input binding lives in the edit branch only and the view
  ...
  region tasks (label, draft, mode) := jsx%
    <li class={if mode == "view" then "task-row" else "task-row editing"}> [
      {if mode == "view"
        ...
        else <input ariaLabel="Task editor" value={draft} onInput={retype}/>},
