...
for (const required of [
  ...
  "    if (regions[0][7][\"length\"] !== 0) {\n      regions[0][0][\"removeMany\"](regions[0][7], null);\n      tx[7][\"push\"](\"region:items:removeMany:\" + regions[0][7][\"length\"]);\n      regions[0][7] = [];\n    }\n    if (regions[0][3]) {\n      regions[0][3] = false;\n      regions[0][0][\"update\"](regions[0][1], null);",
  ...
  // upward, so each `insertAt` addresses a host that already holds every row
  // before it; nothing retained is re-rendered. The dirty branch above zeroes
  // the counter, because a reconcile has already mounted what it counted.
  "      regions[0][4] = [];\n      regions[0][8] = 0;\n    }\n    if (regions[0][8] !== 0) {\n      const append_cursor_0 = [regions[0][1][\"length\"] - regions[0][8]];\n      while (append_cursor_0[0] < regions[0][1][\"length\"]) {\n        regions[0][0][\"insertAt\"](append_cursor_0[0], regions[0][1][append_cursor_0[0]], null);\n        tx[7][\"push\"](\"region:items:insertAt\");\n        append_cursor_0[0] += 1;\n      }\n      regions[0][8] = 0;\n    }",
  ...
  // and the filter table together. ADR-0098 leaves this one on the dirty
  // bit: a hydrate arrives as a whole table into an empty region, where the
  // reconcile clears and refills an owned parent detached.
  ...
  // two region-local slots behind the pending slot. The ADR-0051 filter slot
  // used to hold the container element behind them and ADR-0102 removed it —
  // the sweep writes through the handle now, which has the container already
  // — so the ADR-0097 drops queue, the ADR-0098 append counter and the
  // ADR-0099 accumulator each moved down by one and the record is one slot
  // shorter. The ADR-0062 label count joins the count slots: its ref sits
  // between the predicate count and the total in view order, and its cache
  // slot mounts as the else string — the mounted DOM text of the empty
  // region.
  "const regions = [[region_0, [], 0, false, [], [count_text_19, count_text_20, count_text_22], [0, \" items left\", 0], [], 0, [0, 0, 0]]];",
  ...
  "    const region_touched_0 = regions[0][3] || regions[0][7][\"length\"] !== 0 || regions[0][8] !== 0 || regions[0][4][\"length\"] !== 0;",
  "    const region_structural_0 = regions[0][3] || regions[0][7][\"length\"] !== 0 || regions[0][8] !== 0;",
  ...
  // written. ADR-0102: the write is the handle's `setDisplayed`, taking the
  // row out of the container or putting it back at its table position, with
  // the row's key beside the position for the same LRX-REGION-003 check
  // `removeAt` and `removeMany` carry — no `childAt`, and no container ref
  // on the record for it to navigate from. The written count rides one
  ...
  "    const filter_dirty_0 = regions[0][3];\n    const filter_moved_0 = regions[0][4];\n    const filter_added_0 = regions[0][8];\n    const region_touched_0 = regions[0][3] || regions[0][7][\"length\"] !== 0 || regions[0][8] !== 0 || regions[0][4][\"length\"] !== 0;\n    const region_structural_0 = regions[0][3] || regions[0][7][\"length\"] !== 0 || regions[0][8] !== 0;\n    const region_drain_0_0 = regions[0][3] || regions[0][7][\"length\"] !== 0 || regions[0][8] !== 0 || regions[0][4][\"length\"] !== 0 && action === \"toggle\";\n    const region_drain_0_1 = regions[0][3] || regions[0][7][\"length\"] !== 0 || regions[0][8] !== 0 || regions[0][4][\"length\"] !== 0 && (action === \"edit\" || action === \"commit\" || action === \"keys\");\n    if (regions[0][3]) {\n      regions[0][9][0] = 0;\n      regions[0][9][1] = 0;\n      regions[0][9][2] = 0;\n      for (const count_row_0 of regions[0][1]) {\n        if (count_row_0[3] === \"false\") {\n          regions[0][9][0] += 1;\n        }\n        if (count_row_0[3] === \"true\") {\n          regions[0][9][1] += 1;\n        }\n        if (count_row_0[4] === \"edit\") {\n          regions[0][9][2] += 1;\n        }\n      }\n      tx[7][\"push\"](\"predicate:items:read:\" + regions[0][1][\"length\"]);\n    }\n    if (region_drain_0_0) {\n      tx[5] += 1;\n      tx[7][\"push\"](\"count:items:0:evaluated\");\n      const count_next_0_0 = regions[0][9][0];",
  "    }\n    if (region_drain_0_1) {\n      tx[8] += 1;\n      tx[7][\"push\"](\"attr:6:hidden:evaluated\");\n      const attr_next_6 = regions[0][9][2] === 0;\n      const attr_changed_6 = attrCache[6] !== attr_next_6;",
  ...
  "      const count_next_0_0 = regions[0][9][0];",
  "      const count_next_0_1 = regions[0][9][0];",
]) {
